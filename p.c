...
K1(lib){init(x);r0(K(".[`.P;();:;]",dld(x)));K r,d0=K("\\d");S s[]={"\\d .P","e:{runs[0;x];}","k)eval:runs[1]","k)scalar:py2q call[eval\"lambda x:x.tolist()\";;()!()]@,:","k)py2q:{$[isp x;conv type[x]0;]x}",
 "k)dict:{({$[&/10=@:'x;`$;]x}py2q key x)!py2q value x}","k)conv:((- 1 3 7 9 21 30h)!getb,getnone,getj,getf,repr,scalar),4 10 30 41 42 99h!getG,getC,{d#x[z;0]1*/d:y z}[getarray;getarraydims],(2#(py2q'getseq@)),dict",0};
