...
K1(lib){init(x);r0(K(".[`.o;();:;]",dld(x)));K r,d0=K("\\d");S s[]={"\\d .o","e:{runs[0;x];}","k)eval:runs[1]","pc:{'[call[x;;()!()];enlist]}","scalar:pc[eval\"lambda x:x.tolist()\"]","py2q:{$[isp x;conv type[x]0;]x}","k)dict:{({$[&/10=@:'x;`$;]x}py2q key x)!py2q value x}","k)conv:((- 1 3 7 9 21 30h)!getb,getnone,getj,getf,repr,scalar),4 10 30 41 42 99h!getG,getC,{d#x[z;0]1*/d:y z}[getarray;getarraydims],(2#(py2q'getseq@)),dict",0};
 S*p=s-1;while(*++p)r0(Ke(*p));kpy2q=Ke("py2q");r=Ke("k)(k,`c)!(.o k:`peval`e`import`get`set`call`key`value`getattr`isp`type`py2q`q2py`repr`conv`runs),,.o");
