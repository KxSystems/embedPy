...
Z O ck(O x,O y){K a=ko(y);Py_INCREF(y);K r=K(".",r1(pget(x)),py2q(a));O o=ok(r);R r0(r),o;} // ok here is fine without checking, python will print pyerr
...
Z K kpy2q;Z K1(py2q){R K("@",r1(kpy2q),x);}
...
#define Ke(x) {K r=K(x);if(r->t==-128)R fprintf(stderr,"  %s\n",x),r;}
K1(lib){init(x);r0(K(".[`.o;();:;]",dld(x)));K r,d0=K("\\d");r0(K("\\d .o"));
 Ke("e:{runs[0;x];};eval:runs[1]")
 Ke("pc:{'[call[x;;()!()];enlist]}")
 Ke("scalar:pc[eval\"lambda x:x.tolist()\"]")
 Ke("py2q:{$[isp x;conv type[x]0;]x}")
 kpy2q=K("py2q");
 Ke("dict:{({$[&/10=@:'x;`$;]x}py2q key x)!py2q value x}")
 Ke("conv:((- 1 3 7 9 21 30h)!getb,getnone,getj,getf,repr,scalar),4 10 30 41 42 99h!getG,getC,{d#x[z;0]1*/d:y z}[getarray;getarraydims],(2#(py2q'getseq@)),dict");
 r=K("k!.o k:`eval`e`import`get`set`call`key`value`getattr`isp`type`py2q`q2py`repr`conv`runs");
 K(".\"\\d \",$:",d0);
 R r;
}
